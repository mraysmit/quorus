...
**License:** Apache 2.0
**Status:** Active — actionable breakdown of the [2026-09-24 documentation review](../QUORUS_DOCUMENTATION_REVIEW_2026-09-24.md)
...
[Quorus Documentation Review — 2026-09-24](../QUORUS_DOCUMENTATION_REVIEW_2026-09-24.md) into
...
register (register §12.3). Tasks marked **→ Register** need that step.
...
| Section | Area | Tasks | Effort (review estimate) |
|---|---|---|---|
| 3 | Decisions needed first | 5 | — |
| 4 | Phase A — correctness and safety | 7 | 2–3 days |
| 5 | Phase B — reconcile the canonical set | 7 | 3–5 days |
| 6 | Phase C — consolidate and archive | 10 | about 2 days |
| 7 | Phase D — automation and governance | 5 | ongoing |
| 8 | Code defects outside Phases A–D | 11 | not estimated |
| 9 | Document fixes outside Phases A–D | 17 | not estimated |
...
|---|---|---|---|---|---|
| **DR-A1** | H | Commit the 2026-09-07 work: R1-1 tests (`ContainerRecreationDurabilityTest`, `DockerComposeCluster`) and evidence, `docker-compose-3node-durable.yml` and the other hardened fixtures, the `LeaderGuardHandlerTest` fix, the staged `TransferMetrics` deletion, plan v1.26 and register v1.3. Then add the commit SHA, timestamps and log hashes to the r1 evidence, and resolve its fixture-durability contradiction (§3 and §10.4 against §12.1). | `docs-design/task/`, `docs-design/evidence/r1-container-recreation-2026-09-07.md`, `quorus-controller/src/test/` | Committed as `804e11d` (R1-1, evidence, fixtures, plan/register and test correction) and `3343785` (`TransferMetrics` removal); the R1 evidence cites the reachable revision. Retained raw-log hashes remain to be recorded if available. | 🟨 |
...
| **DR-B7** | **Plan and register** (⏸ DR-A1). Settle the Phase 0 and Phase 4 status in one place and explain the "reopened" note. Correct the present-tense "no volumes" statements. Re-verify OBS-04, OBS-05, OBS-07 (37, not 53) and OBS-14. Fix the section counts (F: 10 rows; H: 0 open) and the revision order, and cite plan v1.26. Add IDs for the three orphan plan items (durable agent-report outbox, the two Raft regression cases, the persistent-environment inventory) and the four configuration residuals (§6 #16). Add the plan revision history that §23 requires, a `**Status:**` line for Phases 5–12, and remove the RocksDB coverage claim (line 384) and the machine path (line 116). Reconcile DEF-09 with DOC-06, the OTel plan version (v2.5 vs v2.6), and OBS-08. | `docs-design/task/` | ⬜ |
...
| **DR-C2** | Move to `docs-design/archive/`, each with a one-line "why archived" banner: the three `CONNECTION_POOL_*` documents; `VERTX5_PERFORMANCE_BENCHMARKS.md`; all six `vertx-migration/` files (keep SUMMARY as the one migration record); `testing/FTPS_INTEGRATION_TEST_INVESTIGATION.md` (mark it resolved first); `evidence/remediation-r4-r6-2026-09-05.md`; and the Configuration Handover (⏸ extract §2.1 and §8 into the configuration reference first, see DR-D1). | ⬜ |
...
| **DR-C10** | Clean the working copy: delete the local `temp/` worktrees, `.history/` and `hs_err_pid*.log`, and untrack the five `temp/*.txt` files still in git. Normalise line endings with a `* text=auto` rule, committed on its own. Merge NOTICE and OPEN_SOURCE_USAGE into one generated inventory (see DR-F01). | ⬜ |
...
These §6 defects are not covered by any §9 action. Each is delivery work: add it to the enterprise
plan and register first (**→ Register**), and deliver it under the plan's §6.1 TDD protocol.
...
| **DR-F07** | `docs/QUORUS_CODEBASE_AND_DOCUMENTATION_REVIEW_2026-08-31.md` | Append the QR-01 to QR-11 status table from review §5. Create `docs-design/reviews/` and move this review and the 2026-09-24 review into it. Fix the GitHub line anchors, which do not resolve in rendered Markdown. | ⬜ |
| **DR-F08** | `docs-design/README.md` | Bump the header date to match the body, add `evidence/`, `architecture-decisions/`, `reference/` and `reviews/` to the directory table, and add this task list. | ⬜ |
...
| **DR-F13** | `QUORUS_CONFIGURATION_ISOLATION_HANDOVER_2026-09-03.md` | Before archiving (DR-C2): fix the broken `../../raftlog/pom.xml` link and remove the personal path. The six orphaned SHAs are covered by DR-A2. | ⬜ |
| **DR-F14** | All live documents | Remove `C:\Users\mraysmit\…` paths (README, plan, Configuration Handover, Raft Cluster Testing). | ⬜ |
