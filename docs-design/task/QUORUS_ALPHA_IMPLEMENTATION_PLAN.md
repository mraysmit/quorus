...
- Historical factory alternatives have been removed: `RaftStorageFactory` now supports only the external raftlog library. See the current enterprise remediation checkpoint.
...
| `QUORUS_RAFT_STORAGE_TYPE` | Storage backend: `raftlog` only | `raftlog` |
