...
NODE_ID=${QUORUS_NODE_ID:-${NODE_ID:-$DEFAULT_NODE_ID}}
RAFT_PORT=${QUORUS_RAFT_PORT:-${RAFT_PORT:-$DEFAULT_RAFT_PORT}}
RAFT_HOST=${RAFT_HOST:-$DEFAULT_RAFT_HOST}
ELECTION_TIMEOUT_MS=${QUORUS_RAFT_ELECTION_TIMEOUT_MS:-${ELECTION_TIMEOUT_MS:-$DEFAULT_ELECTION_TIMEOUT_MS}}
HEARTBEAT_INTERVAL_MS=${QUORUS_RAFT_HEARTBEAT_INTERVAL_MS:-${HEARTBEAT_INTERVAL_MS:-$DEFAULT_HEARTBEAT_INTERVAL_MS}}
CLUSTER_NODES=${QUORUS_CLUSTER_NODES:-${CLUSTER_NODES:-}}
...
if [ -z "$CLUSTER_NODES" ]; then
    echo "ERROR: QUORUS_CLUSTER_NODES (or legacy CLUSTER_NODES) environment variable is required"
